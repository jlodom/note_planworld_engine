...
class User {
  ...
  /**
   * JLO2 20191005 - A simplified form of getPlan that does not support the Archive pieces.
   * Implemented so that we would have enough functionality to start building v3 around.
   */
  function getPlanSimple (&$user) {
    if ($user->getUserID() != $this->userID) {
      $this->addView();
      // VERY IMPORTANT TO ADD THIS BACK IN WHEN IMPLEMENTING ARCHIVE JLO2 20191005
    ...
    if (!isset($this->plan)) {
      try{
        ...
        $query = $this->dbh->prepare("SELECT content FROM plans WHERE uid=:uid");
        $queryArray = array('uid' => $this->userID);
        $query->execute($queryArray);
        $result = $query->fetch();
        if (!$result){
  ...
  function setPlanSimple ($plan) {
    ...
    try{
      ...
      $query1 = $this->dbh->prepare('DELETE FROM plans WHERE uid=:uid');
      $queryArray1 = array('uid' => $this->userID);
    ...
    $thisSnoop = new Snoop();
    $boolSuccess = $boolSuccess && $thisSnoop->process($this, $plan, $oldplan);
  ...
  /* JLO2 20190830 as of today here are the functions I think are missing.
  Not all of them need to be implemented, but certainly the Snitch and Snoop bits
  Then we can actually, you know, display a plan.
